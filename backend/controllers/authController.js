...
export async function registerUserController(req, res) {
  try {
    ...
    if (existingUser) {
      return res.status(400).json({ message: "Email already registered confirmed from auth.controller", error: true, success: false });
    }
...
//     await redisClient.setEx(token, 60 * 60 * 24, 'blacklisted'); // 24h expiry
